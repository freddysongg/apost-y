...
export function useHotkeys(): void {
  ...
  useEffect(() => {
    ...
    if (keybindService.isElectron()) {
      if (!window.electronAPI) return;

      const unsubscribe = window.electronAPI.hotkey.onHotkeyEvent((action: string) => {
        const actionHandlers: Record<string, () => void> = {
          ...
          toggleSystemAudio,
          pushToTalk: () => {
            const { inputMode: currentMode } = useSessionStore.getState();
            if (currentMode === 'push-to-talk') {
              const { isPushToTalkActive: currentPtt } = useSessionStore.getState();
              setPushToTalkActive(!currentPtt);
            }
          },
        };
